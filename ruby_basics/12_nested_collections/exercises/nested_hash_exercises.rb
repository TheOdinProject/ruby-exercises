...
def find_beautiful_languages(languages)
  ...
  # key/value pair { is_beautiful?: true } listed in their information
    res = languages.select {|lang, pair| pair.dig(:is_beautiful?) && pair[:is_beautiful?]}
end
...
def find_language_facts(languages, language_name, fact_index = 0)
  ...
  #                 is_beautiful?: false }
  # }
  return nil if languages.dig(language_name).nil?
  res = languages[language_name][:facts][fact_index]
end
